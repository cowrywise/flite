...
from flite.account.test.factories import CardFactory, \
    BankFactory
...
class TestUserListTestCase(APITestCase):
    ...
    def test_post_request_with_valid_data_succeeds_and_profile_and_account_is_created(self):
        response = self.client.post(self.url, self.user_data)
        eq_(response.status_code, status.HTTP_201_CREATED)

        eq_(UserProfile.objects.filter(user__username=self.user_data['username']).exists(),True)
        eq_(Account.objects.filter(owner__username=self.user_data['username']).exists(),True)
...
class TestDepositTestCase(APITestCase):
    ...
    def test_user_can_deposit_with_card_and_bank(self):
        ...
        old_bal = Account.objects.get(owner=self.testuser1).available_balance
        payload = {'amount': amount, 'card': self.card.id, 'bank': self.bank.id}
        response = self.client.post(self.url, payload)
...
class TestWithdrawTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.url2 = reverse('user-withdrawal', kwargs={'pk': self.testuser2.pk})
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.testuser1.auth_token}')
