...
class TransactionsListViewTests(APITestCase):
    ...
    def test_get_transactions_list(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 5)  # Check if the default number of transactions returned is 5
    ...
    def test_nonexistent_account_id(self):
        url = reverse('transactions-list', kwargs={'account_id': '00000000-0000-0000-0000-000000000000'})  # Nonexistent account ID
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data, {'detail': 'Not found.'})  # Check if proper 404 response is returned
...
class TransactionDetailViewTests(APITestCase):
    ...
    def test_get_transaction_detail(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['owner'], str(self.user.id))  # Check if the owner ID matches
