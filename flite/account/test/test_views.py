...
class TestTransferTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('account-transfers', kwargs={'pk': self.account1.pk, 'receipient_account_id': self.account2.id})
        self.url2 = reverse('account-transfers', kwargs={'pk': self.account2.pk, 'receipient_account_id': self.account1.id})
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.testuser1.auth_token}')

    def test_user_can_p2ptransfer(self):
        amount = fake.random_int(min=100, max=1000)
        Account.objects.filter(owner=self.testuser1).update(available_balance=self.old_bal)

        response = self.client.post(self.url, {'amount': amount})
        eq_(response.status_code, status.HTTP_202_ACCEPTED)

        testuser1_new_bal = Account.objects.get(owner=self.testuser1).available_balance
        eq_(self.old_bal - testuser1_new_bal , float(amount))

        testuser2_new_bal = Account.objects.get(owner=self.testuser2).available_balance
        eq_(testuser2_new_bal, float(amount))
    
    def test_user_can_p2ptransfer_with_invalid_details(self):
        amount = fake.random_int(min=100, max=1000)
        fake_account_id = fake.uuid4()
        url = reverse('account-transfers', kwargs={'pk': self.account1.pk, 'receipient_account_id': fake_account_id})
        Account.objects.filter(owner=self.testuser1).update(available_balance=self.old_bal)
    ...
    def test_user_can_p2ptransfer_with_owned_account_only(self):
        amount = fake.random_int(min=100, max=1000)
        Account.objects.filter(owner=self.testuser2).update(available_balance=self.old_bal)
...
class TestTransactionsListTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('account-transactions', kwargs={'pk': self.account.pk, })
        self.url2 = reverse('account-transactions', kwargs={'pk': self.account1.pk, })
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.testuser1.auth_token}')
        


    def test_user_can_get_transaction_list(self):
        Account.objects.filter(owner=self.testuser1).update(available_balance=3000)

        self.client.post(
            reverse('account-transfers', 
            kwargs={'pk': self.account.pk, 'receipient_account_id': self.account1}),
            {'amount': 200})
        ...
        transaction_count = Transaction.objects.filter(owner=self.testuser1).count()
    ...
    def test_user_can_get_owned_transaction_list_only(self):
        ...
        transaction_count = Transaction.objects.filter(owner=self.testuser1).count()
        transaction_count2 = Transaction.objects.filter(owner=self.testuser2).count()
...
class TestTransactionDetailTestCase(APITestCase):
    ...
    def test_user_can_get_transaction(self):
        ...
        url = reverse(
            'account-transaction', 
            kwargs={'pk': self.account.pk, 'transaction_id': transaction.id})
    ...
    def test_user_can_get_transaction_with_invalid_details(self):
        ...
        url = reverse(
            'account-transaction', 
            kwargs={'pk': self.account1.pk, 'transaction_id': transaction_id})
